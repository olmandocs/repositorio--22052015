$(document).on('pageinit', '#add-contact', function(){ //se invoca solo cuando la pagina es add contact, reemplaza a pagecreate
	...
	$('#save-contact').on('click', function(){
		...
		function onConfirm(buttonIndex){
			...
			if (buttonIndex === 1) {
				...
				navigator.notification.alert( //api dialogs cordova
		...
		function onSuccess(contact){
			...
			navigator.notification.confirm( //api dialogs cordova
		...
		var contact = navigator.contacts.create();//api contacts cordova
		...
		var emails = [];//siempre es un arreglo
		...
		var mobile_numbers = [];//siempre es un arreglo
